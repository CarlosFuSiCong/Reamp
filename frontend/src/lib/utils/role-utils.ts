...
export function canManageAgencyMember(
  userRole: AgencyRole | undefined | null,
  ...
  return (
    targetRole !== AgencyRole.Owner &&
    (userRoleValue === AgencyRole.Owner || userRoleValue === AgencyRole.Manager)
  );
}

export function canManageStudioMember(
  userRole: StudioRole | undefined | null,
  targetRole: StudioRole
): boolean {
  if (userRole === undefined || userRole === null) return false;
  const userRoleValue = typeof userRole === "string" ? parseInt(userRole, 10) : Number(userRole);
  return (
    targetRole !== StudioRole.Owner &&
    (userRoleValue === StudioRole.Owner || userRoleValue === StudioRole.Manager)
  );
}
