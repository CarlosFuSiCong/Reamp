...
export default function EditListingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: listing, isLoading, error } = useListing(resolvedParams.id);
  ...
  const onSubmit = async (data: ListingFormValues) => {
    ...
    const apiData = {
      ...
      address: {
        ...
        country: "AU", // Convert to ISO code
      },
  ...
  if (error) {
    return <ErrorState message="Failed to load listing" />;
  }

  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <PageHeader title="Edit Listing" description="Update property listing details" />
