...
export function StudioRoleBadge({ role, className }: StudioRoleBadgeProps) {
  if (role === undefined || role === null) {
    return (
      <Badge className={cn("bg-muted/50 text-foreground border-border", className)}>Staff</Badge>
    );
  }
  ...
  const colorClass =
    colorClasses[roleValue as StudioRole] || "bg-muted/50 text-foreground border-border";
